...
export async function generateStaticParams() {
  const slugs = getAllSlugs('examples');
  return slugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const page = getContentBySlug('examples', slug);
  if (!page) return { title: 'Not Found' };
  return {
    title: `${page.frontmatter.title} — SRP Examples`,
    description: page.frontmatter.description,
  };
}

export default async function ExamplesPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const page = getContentBySlug('examples', slug);
  if (!page) notFound();

  const section = getSectionByKey('examples')!;
  const { prev, next } = getGlobalPrevNext('examples', slug);
