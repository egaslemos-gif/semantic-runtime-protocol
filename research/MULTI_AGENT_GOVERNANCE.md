...
## The Semantic OS Capability Matrix
The Semantic Runtime Protocol (SRP) centralizes governance. All agents must fetch their context from the same Semantic OS Singleton.
...
By routing all requests through the `CapabilityGate` and the SRP `Adapters`, the Semantic OS ensures that while each agent receives a tailored payload shape, the *underlying operational truth* (the Governance Edges) is mathematically identical across all of them.
