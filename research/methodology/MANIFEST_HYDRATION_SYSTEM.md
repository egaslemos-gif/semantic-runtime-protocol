...
2. **Adjacency Re-Linking:** The manifest stores edges as string references (e.g., `enforces:system.auth`). Hydration MUST re-link these strings back into in-memory object references (pointers) so that O(1) pointer traversal works during queries.
