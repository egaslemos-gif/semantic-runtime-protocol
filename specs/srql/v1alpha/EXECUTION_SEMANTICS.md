...
When `GET governance FOR system.auth` is executed, the Engine guarantees a strict order of edge resolution (Breadth-First Search priority queue):
...
```sql
GET governance FOR system.auth MAX_DEPTH 9999 BUDGET 100000
```
...
GET governance FOR system.auth MAX_DEPTH 2 BUDGET 200
```
The client receives a `206 Partial Content` or equivalent flag indicating the traversal was forcibly bounded by the Hypervisor.
